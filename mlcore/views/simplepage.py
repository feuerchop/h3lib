...
class Mails(object):
    ...
    def __init__(self):
        ...
        self.GLOVE_SET = './datasets/glove.twitter.27B.25d.json'
    ...
    # TODO: RESTful using cp_dispatch
    ...
    #
    # @cherrypy.expose
    # def testcp(self, name):
    #     cherrypy.log('TESTCPPPPPPPP >>>>> ' + name)
    ...
    def retrainModel(self):
        ...
        fw.close()
        cherrypy.log("[INFO] New model is trained and saved in {} ... OK".format(''.join([self.TRAIN_MODEL_PATH, 'DlgActTypeClf.p'])))
    ...
    def buildWordEmbedding(self, pretrained=False):
        ...
        if pretrained is False and os.path.exists(SAVE_FILE + '.txt') is False:
            ...
            os.system("{}build/vocab_count -min-count {} -verbose {} < {} > {}".format(GLOVE_PATH,
                                                                                       ...
                                                                                       VOCAB_FILE))
            # 2. words coocurance
            os.system("{}build/cooccur -memory {} -vocab-file {} -verbose {} -window-size {} < {} > {}".format(GLOVE_PATH, MEMORY, VOCAB_FILE, VERBOSE, WINDOW_SIZE, CORPUS, COOCCURRENCE_FILE))
            # 3. shuffle
            ...
            os.system(
                "{}build/glove -save-file {} -threads {} -input-file {} -x-max {} -iter {} -vector-size {} -binary {} -vocab-file {} -verbose {}".format(
                    GLOVE_PATH, SAVE_FILE, NUM_THREADS, COOCCURRENCE_SHUF_FILE, X_MAX, MAX_ITER, VECTOR_SIZE, BINARY, VOCAB_FILE, VERBOSE))
...
if __name__ == '__main__':
    ...
    cherrypy.quickstart(Mails(), '/', conf)
